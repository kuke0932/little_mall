...
Page({
  ...
  onShareAppMessage: function() {
    ...
    return {
      ...
      desc: '富锌鹌鹑蛋',
      path: '/pages/goods/goods?goodId=' + that.data.id
    }
  },
    ...
    util.request(api.GoodsDetail, {
      ...
      if (res.errno === 0) {
        ...
        that.setData({
          ...
          miniPicUrl: res.data.specificationList[0].valueList[0].picUrl
        });
  ...
  previewImg: function(event) {
    wx.previewImage({
      urls: [this.data.miniPicUrl]
    })
  },

  // 规格选择
  clickSkuValue: function(event) {
    if (event._relatedInfo.anchorTargetText === '成人A型') {
      this.setData({
        miniPicUrl: this.data.specificationList[0].valueList[0].picUrl
      })
    } else {
      this.setData({
        miniPicUrl: this.data.specificationList[0].valueList[1].picUrl
      })
    }
    let that = this;
    let specName = event.currentTarget.dataset.name;
  ...
  onLoad: function(options) {
    ...
    // this.data.specificationList[0].valueList[0].checked = true
    // this.setData({
